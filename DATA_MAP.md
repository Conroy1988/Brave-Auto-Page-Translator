...
| --- | --- | --- | --- | --- | --- |
| Readable webpage text and writing explicitly submitted through Smart Compose/workspace | Return the translation requested or authorized by the user | Original page text exists only in tab memory for restoration; a bounded cache and up to 20 recent translations per tab exist only in background memory | To the browser on-device engine, or over HTTPS to an explicitly approved external provider | Tab data ends on navigation/close; cache/history end on background restart or user clear | **Website content** and **user-provided content** — handled and transmitted for core functionality when an external provider is used |
...
| Approved/excluded sites, per-site targets, site-intelligence profiles and language exclusions | Apply user-created translation rules | Local extension storage only | No | Until removed, reset or extension uninstalled | Describe as local app functionality; not browsing-history collection |
...
| User-defined Privacy Firewall terms | Mask exact confidential values before an external translation request | Local extension storage only; deliberately excluded from settings backups | No; matching values are replaced with local protection tokens | Until removed, reset or extension uninstalled | App functionality/privacy; values are not separately uploaded |
...
- Verify Smart Compose excludes login, password, passcode, PIN, one-time-code, security-code and payment fields and requires preview confirmation before replacement.
- Verify Privacy Firewall tokens fail closed and route metrics contain counts only, never the protected values.
